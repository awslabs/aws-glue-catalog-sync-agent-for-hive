...
public class HiveGlueCatalogSyncAgent extends MetaStoreEventListener {
	...
	private ObjectQueue<String> ddlQueue;
							...
							try {
								...
								if (e instanceof SQLRecoverableException || e instanceof SQLTimeoutException) {
									...
									if (e.getMessage().contains("AlreadyExistsException") && dropTableIfExists) {
										...
										cwlr.sendToCWL("Unable to complete query: " + query);
										cwlr.sendToCWL("ERROR: " + e.getMessage());
									}
									iterator.remove();
								}
	...
	static class StringConverter implements ObjectQueue.Converter<String> {
		public String from(byte[] bytes) throws IOException {
			return new String(bytes, "UTF-8");
		}

		public void toStream(String s, OutputStream os) throws IOException {
	...
	public HiveGlueCatalogSyncAgent(final Configuration conf) throws Exception {
		...
		// Initializing QueueFile
		File file = new File("object-queue");
		QueueFile queueFile = new QueueFile.Builder(file).build();
		ddlQueue = ObjectQueue.create(queueFile, new StringConverter());
